...
export class Raycaster {
    ...
    setFromCameraAndScreenPosition = (camera, screenPosition, screenDimensions) => {
        this.ndcCoords.x = screenPosition.x / screenDimensions.x * 2.0 - 1.0;
        this.ndcCoords.y = (screenDimensions.y - screenPosition.y) / screenDimensions.y * 2.0 - 1.0;
        if (camera.isPerspectiveCamera) {
            this.ray.origin.setFromMatrixPosition(camera.matrixWorld);
            this.ray.direction.set( this.ndcCoords.x,  this.ndcCoords.y, 0.5 ).unproject(camera).sub(this.ray.origin).normalize();
            this.camera = camera;
        } else if (camera.isOrthographicCamera) {
            this.ray.origin.set( this.ndcCoords.x,  this.ndcCoords.y,
